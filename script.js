...
function pageContext(){
  ...
  const empreendimento=
    ...
    document.body.dataset.empreendimento||
    ((rawTitle && !/^Empreendimentos em Campinas/i.test(rawTitle))
      ?rawTitle
      :'Karen Caroline Imóveis');
...
async function saveLead(payload){
  ...
  try{
    ...
    const err=new Error(
      queued
        ?'CRM indisponível agora. O pedido foi salvo neste navegador e será sincronizado quando o site estiver conectado.'
...
function campaignLabel(){
  ...
  if(u.utm_source)
    parts.push(`origem: ${u.utm_source}`);

  if(u.utm_campaign)
    parts.push(`campanha: ${u.utm_campaign}`);

  if(u.utm_medium)
    parts.push(`mídia: ${u.utm_medium}`);
...
for(const form of document.querySelectorAll('.lead-form')){
  ...
  form.addEventListener('submit',async e=>{
    ...
    const box=
      form.parentElement.querySelector('.form-ok');
...
<path
  fill="currentColor"
  d="M19.11 17.24c-.3-.15-1.77-.87-2.04-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.17-.17.2-.35.22-.65.07-.3-.15-1.27-.47-2.42-1.5-.9-.8-1.5-1.78-1.68-2.08-.17-.3-.02-.46.13-.61.14-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.62-.92-2.22-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.8.37-.27.3-1.04 1.02-1.04 2.49s1.07 2.89 1.22 3.09c.15.2 2.1 3.2 5.08 4.49.71.31 1.26.49 1.69.63.71.23 1.36.19 1.87.12.57-.08 1.77-.72 2.02-1.42.25-.7.25-1.3.17-1.42-.07-.12-.27-.2-.57-.35z"
/>
<path
  fill="currentColor"
  d="M16.05 3.2c-7.1 0-12.87 5.74-12.87 12.82 0 2.26.59 4.47 1.72 6.41L3.07 29l6.75-1.77a12.9 12.9 0 0 0 6.22 1.58h.01c7.1 0 12.87-5.74 12.87-12.82 0-3.43-1.34-6.65-3.77-9.07A12.83 12.83 0 0 0 16.05 3.2zm0 23.45h-.01a10.72 10.72 0 0 1-5.46-1.49l-.39-.23-4 .99 1.07-3.89-.25-.4a10.62 10.62 0 0 1-1.63-5.61c0-5.88 4.79-10.66 10.68-10.66 2.85 0 5.53 1.11 7.54 3.12a10.57 10.57 0 0 1 3.13 7.52c0 5.88-4.79 10.65-10.68 10.65z"
/>
</svg>`;

for(
  const icon
  of document.querySelectorAll('.wa-icon')
){
  icon.innerHTML=WHATSAPP_SVG;
}
...
document
  .querySelectorAll(
    '[data-whatsapp-float],.js-whatsapp,[data-whatsapp-link]'
  )
  .forEach(el=>{
    ...
    el.addEventListener('click',e=>{
      ...
      openWhatsApp(
        el.hasAttribute('data-whatsapp-float')
          ?'botao_flutuante'
          :'botao_contextual'
      );
...
function createVisitUI(){
  ...
  btn.type='button';
  btn.className='schedule-float';

  btn.innerHTML=`
    <span class="schedule-dot">⌂</span>
    <span>
      <strong>Agendar visita</strong>
      <small>Escolha o melhor horário</small>
    </span>
  `;
  ...
  wrap.className='visit-backdrop';

  wrap.dataset.globalVisitUi='1';
  ...
  wrap.innerHTML=`
  <div
    class="visit-modal visit-modal-pro"
    role="dialog"
    aria-modal="true"
  ...
  >
    ...
    <div class="visit-grid">
      ...
      <div class="visit-copy">
        ...
        <div class="visit-form-view">

          <p class="eyebrow dark visit-eyebrow">
            AGENDAR VISITA
          </p>

          <h2 id="visit-title">
            Conheça o ${ctx.empreendimento}.
          </h2>

          <p class="visit-intro">
            Deixe seus dados e sua preferência de dia e período.
            O pedido ficará registrado no CRM e a corretora entrará
            em contato para confirmar os detalhes da visita.
          </p>

          <div
            class="visit-interest-note"
            hidden
          ></div>
          ...
          <form class="visit-form">
            ...
            <div class="two-cols visit-schedule-row">
              ...
              <label class="visit-date-field">
                ...
                <input
                  ...
                  name="data_visita"
                  type="date"
                  aria-label="Data preferida"
                >

              </label>

              <label class="visit-period-field">
            ...
            <label class="consent modal-consent">
              ...
              <span>
                Autorizo o contato da corretora para tratar deste
                agendamento. Li a
                <a
        ...
        >
          ...
          <p>
            Recebemos sua solicitação para conhecer o
            <strong>
              ${ctx.empreendimento}
            </strong>.
            Karen Caroline entrará em contato para confirmar
            o melhor horário.
          </p>
          ...
          <div class="success-actions">
            ...
            <button
              type="button"
              class="btn success-wa"
            >
              Falar com Karen no WhatsApp
            </button>
  ...
  const formView=
    wrap.querySelector('.visit-form-view');

  const successView=
    wrap.querySelector('.visit-success');

  const interestNote=
    wrap.querySelector('.visit-interest-note');
  ...
  const open=(interest='')=>{
    ...
    if(selectedInterest){
      ...
      interestNote.textContent=
        `Interesse selecionado: ${selectedInterest}`;

    }else{

      interestNote.hidden=true;
  ...
  wrap
    ...
    .addEventListener(
      ...
      ()=>{
        ...
        const message=
          whatsappMessage(
            `Olá! Acabei de registrar uma solicitação de visita para ${ctx.empreendimento}${selectedInterest?` (${selectedInterest})`:''}.`
          );
...
if(floorLightbox){
  ...
  const img=
    floorLightbox.querySelector('img');

  const title=
    floorLightbox.querySelector(
  ...
  const openFloor=(el)=>{
    ...
    img.src=
      el.dataset.floorplanImage;

    title.textContent=
      el.dataset.floorplanTitle||
      'Planta do empreendimento';

    floorLightbox.classList.add('open');

    floorLightbox.setAttribute(
      'aria-hidden',
      'false'
    );
  ...
  const closeFloor=()=>{
    ...
    floorLightbox.classList.remove(
      'open'
    );
  ...
  floorLightbox.addEventListener(
    'click',
    e=>{

      if(e.target===floorLightbox)
        closeFloor();
  ...
  document.addEventListener(
    ...
    e=>{
      ...
      if(
        e.key==='Escape' &&
        floorLightbox.classList.contains('open')
      ){
        closeFloor();
      }
...
for(
  ...
  b.addEventListener(
    ...
    ()=>{
      ...
      if(window.openVisitModal)
        window.openVisitModal(label);
...
//
// CORREÇÃO:
// Na inicialização usamos show(0,false).
// Dessa forma a primeira imagem é configurada,
// mas scrollIntoView NÃO é executado.
// Assim a página permanece no topo quando o visitante
// entra no empreendimento.
//
(()=>{
  document
    .querySelectorAll('[data-cp-carousel]')
    .forEach(c=>{

      const main=
        c.querySelector('.cp-main');

      const thumbs=[
        ...c.querySelectorAll('.cp-thumb')
      ];
      ...
      const show=(
        ...
        // Só rola as miniaturas quando o usuário
        // realmente interagir com o carrossel.
        if(scrollThumb){
      ...
      // IMPORTANTE:
      // Inicializa a primeira imagem SEM rolar
      // automaticamente até o carrossel.
      show(0,false);
